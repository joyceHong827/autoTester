...
const screenshotDir = path.join(__dirname, 'screenshots', 'issue_49827');

// 情境1 & 4：電視商品提報頁面顯示 ESV 價欄位，並填入 9900 後完整提報
test('B2B TV Product Submission with ESV Price (#49827)', async ({ page }) => {
  // Login using the shared login function
  await b2bLogin(page);
  await page.waitForURL('http://b2b.lab.etzone.net/Web/DefaultPage');

  // Navigate to TV product submission page (etype=2)
  await page.goto('http://b2b.lab.etzone.net/Web/ProductManager?e=2');

  // 【情境1】驗證 ESV 價欄位在電視商品提報頁面可見
  const esvPrice = page.locator('span').filter({ hasText: 'ESV價' }).getByRole('spinbutton');
  await expect(esvPrice).toBeVisible();
  await esvPrice.scrollIntoViewIfNeeded();
  await page.screenshot({ path: path.join(screenshotDir, '01_esv_field_visible.png') });
  ...
  // 【情境4】填入 ESV 價：9900
  await esvPrice.fill('9900');
  await esvPrice.scrollIntoViewIfNeeded();
  await page.screenshot({ path: path.join(screenshotDir, '02_esv_price_filled.png') });
  ...
  // 5. Final submit clicks - wait for SweetAlert dialog to appear
  await page.getByRole('button', { name: ' 送審' }).click();
  const confirmBtn = page.getByRole('button', { name: '送審', exact: true });
  await expect(confirmBtn).toBeVisible({ timeout: 10000 });
  await page.screenshot({ path: path.join(screenshotDir, '03_submit_confirm_dialog.png') });
  await confirmBtn.click();

  // 截圖：提報完成後的結果畫面
  await page.waitForTimeout(2000);
  await page.screenshot({ path: path.join(screenshotDir, '03_submission_result.png') });
});

// 情境2：非電視商品（etype != 2）不顯示 ESV 價欄位
test('B2B Non-TV Product should NOT show ESV Price field (#49827)', async ({ page }) => {
  await b2bLogin(page);
  await page.waitForURL('http://b2b.lab.etzone.net/Web/DefaultPage');

  // Navigate to a non-TV product page (etype=1 一般商品)
  await page.goto('http://b2b.lab.etzone.net/Web/ProductManager?e=1');

  // 截圖：確認非電視商品頁面不含 ESV 價欄位
  await page.screenshot({ path: path.join(screenshotDir, '04_non_tv_no_esv_field.png') });

  // 預期結果：非電視商品提報頁面不顯示「ESV價」欄位
  const esvPrice = page.locator('span').filter({ hasText: 'ESV價' }).getByRole('spinbutton');
  await expect(esvPrice).not.toBeVisible();
});